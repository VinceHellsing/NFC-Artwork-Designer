...
const DPI = 300; // High-quality DPI for professional sticker printing
...
const BLEED_PX = 30; // The space from the cut line to the start of the design (scaled for 300 DPI)
...
function App() {
  ...
  const drawCardOnCanvas = useCallback(
    (ctx, card, options = {}) => {
      ...
      if (card.template === "steamRetro" || card.template === "gameCard") {
        ...
        if (card.outerFrameDesign === "wateryBlue") {
          ...
          for (let i = 0; i < 150; i++) {
            ...
            if (Math.random() > 0.5) {
              x =
                Math.random() > 0.5
                  ? frameX + Math.random() * 20
                  : frameX + frameW - Math.random() * 20;
              y = Math.random() * frameH + frameY;
            } else {
              x = Math.random() * frameW + frameX;
              y =
                Math.random() > 0.5
                  ? frameY + Math.random() * 20
                  : frameY + frameH - Math.random() * 20;
            }
            ctx.beginPath();
            ctx.moveTo(x, y);
            ctx.lineTo(
          ...
          for (let i = 0; i < 40000; i++) {
            ...
            ctx.fillRect(
              Math.random() * frameW + frameX,
              Math.random() * frameH + frameY,
              Math.random() * 2,
              Math.random() * 2,
            );
          }
        ...
        const FOOTER_H = isGameCard ? 0 : H_design * 0.1;
        ...
        if (!isGameCard) {
          ...
          wrapText(
            ctx,
            card.artistText.text,
            frameX + PADDING,
            FOOTER_Y + PADDING,
            frameW - PADDING * 2,
            card.artistText.fontSize * 1.2,
            card.artistText.align,
          );
          ...
          ctx.textAlign = "right";
          ctx.fillText(
            card.footerText.text,
            frameX + frameW - PADDING,
            FOOTER_Y + FOOTER_H - PADDING,
          );
        }
        ctx.restore();
        ...
        if (card.title.align === "right")
          textX = CONTENT_X + CONTENT_W - PADDING;
        ...
        if (card.template === "modern") {
          ...
          ctx.textBaseline = "middle";
          textX = CONTENT_X + PADDING;
          if (card.artistText.align === "center")
            textX = CONTENT_X + CONTENT_W / 2;
          if (card.artistText.align === "right")
            textX = CONTENT_X + CONTENT_W - PADDING;
          ...
          ctx.textBaseline = "middle";
          ctx.fillText(
            card.cornerText.text,
            CORNER_BOX_X + CORNER_BOX_W / 2,
            CORNER_BOX_Y + CORNER_BOX_H / 2,
          );
        } else {
          ...
          ctx.fillText(
            card.cornerText.text,
            CORNER_BOX_X + CORNER_BOX_SIZE / 2,
            CORNER_BOX_Y + CORNER_BOX_SIZE / 2,
          );
        }
      }
  ...
  const updateCard = (updates) => {
    setCards(
      cards.map((c) => (c.id === activeCardId ? { ...c, ...updates } : c)),
    );
  };
      ...
      .flatMap((card) => {
        ...
        if (card.image)
          promises.push(
            new Promise((resolve, reject) => {
              if (
                loadedImages[card.image] &&
                loadedImages[card.image].complete
              ) {
                resolve({ url: card.image, img: loadedImages[card.image] });
                return;
              }
              const img = new Image();
        ...
        if (card.headerImage)
          promises.push(
            new Promise((resolve, reject) => {
              if (
                loadedImages[card.headerImage] &&
                loadedImages[card.headerImage].complete
              ) {
                resolve({
                  url: card.headerImage,
                  img: loadedImages[card.headerImage],
                });
                return;
              }
              const img = new Image();
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      // Calculate the grid starting position (shared across all pages)
      const gridStartX =
        (pageWidth - (cols * pageSlotW_mm + (cols - 1) * spacing)) / 2;
      const gridStartY =
        (pageHeight - (rows * pageSlotH_mm + (rows - 1) * spacing)) / 2;
      ...
      for (let i = 0; i < cards.length; i++) {
        ...
        const cutLineX = gridStartX + col * (pageSlotW_mm + spacing);
        const cutLineY = gridStartY + row * (pageSlotH_mm + spacing);
      ...
      // Add registration marks on page edges after all cards are placed
      if (showCutLines) {
        const totalPages = pdf.internal.getNumberOfPages();
        const markLength = 5; // Length of registration marks in mm

        for (let pageNum = 1; pageNum <= totalPages; pageNum++) {
          pdf.setPage(pageNum);
          pdf.setDrawColor(0, 0, 0); // Black color for registration marks
          pdf.setLineWidth(0.2);

          // Draw vertical marks for each card column (left and right edge of each card)
          for (let col = 0; col < cols; col++) {
            // Left edge of card
            const xLeft = gridStartX + col * (pageSlotW_mm + spacing);
            pdf.line(xLeft, 0, xLeft, markLength); // Top edge
            pdf.line(xLeft, pageHeight - markLength, xLeft, pageHeight); // Bottom edge

            // Right edge of card
            const xRight =
              gridStartX + col * (pageSlotW_mm + spacing) + pageSlotW_mm;
            pdf.line(xRight, 0, xRight, markLength); // Top edge
            pdf.line(xRight, pageHeight - markLength, xRight, pageHeight); // Bottom edge
          }

          // Draw horizontal marks for each card row (top and bottom edge of each card)
          for (let row = 0; row < rows; row++) {
            // Top edge of card
            const yTop = gridStartY + row * (pageSlotH_mm + spacing);
            pdf.line(0, yTop, markLength, yTop); // Left edge
            pdf.line(pageWidth - markLength, yTop, pageWidth, yTop); // Right edge

            // Bottom edge of card
            const yBottom =
              gridStartY + row * (pageSlotH_mm + spacing) + pageSlotH_mm;
            pdf.line(0, yBottom, markLength, yBottom); // Left edge
            pdf.line(pageWidth - markLength, yBottom, pageWidth, yBottom); // Right edge
          }
        }
      }
  ...
  return (
    <div className="h-screen bg-slate-200 flex flex-col font-sans">
      <header className="bg-slate-800 shadow-lg p-4 flex justify-between items-center flex-shrink-0">
        ...
        <button
          onClick={exportToPDF}
          disabled={isExporting || !isPdfLibReady}
              ...
              <div>
                ...
                <button
                  onClick={() =>
                    updateCard({
                      orientation:
                        activeCard?.orientation === "vertical"
                          ? "horizontal"
                          : "vertical",
                    })
                  }
                  className="w-full p-2 bg-slate-700 border border-slate-600 text-white rounded-lg flex items-center justify-center gap-2 hover:bg-slate-600"
                >
              ...
              {activeCard?.template === "shortcuts" ? (
                ...
                <>
                  <TextControl
                    ...
                    onUpdate={(val) => updateCard({ type: val })}
                  />
                  <TextControl
                    label="Header Right Text"
                    value={activeCard?.cornerText}
                    onUpdate={(val) => updateCard({ cornerText: val })}
                  />
                  <TextControl
                    label="Header Sub-Text"
                    value={activeCard?.description}
                    onUpdate={(val) => updateCard({ description: val })}
                  />
                  {activeCard?.template === "steamRetro" && (
                    <>
                      <TextControl
                        label="Footer Center Text"
                        value={activeCard?.artistText}
                        onUpdate={(val) => updateCard({ artistText: val })}
                      />
                      <TextControl
                        label="Footer Right Text"
                        value={activeCard?.footerText}
                        onUpdate={(val) => updateCard({ footerText: val })}
                      />
                    </>
                  )}
                </>
                ...
                <>
                  <TextControl
                    ...
                    onUpdate={(val) => updateCard({ title: val })}
                  />
                  <TextControl
                    label="Type Text"
                    value={activeCard?.type}
            ...
            <SidebarSection title="Artwork & Style">
                  ...
                  {activeCard?.headerImage && (
                    <div className="mt-4">
                      ...
                      <input
                        ...
                        max="5"
                        step="0.1"
                        value={activeCard.headerImageTransform.scale}
                        onChange={(e) =>
                          updateCard({
                            headerImageTransform: {
                              ...activeCard.headerImageTransform,
              ...
              <div>
                ...
                {activeCard?.image && (
                  <div className="mt-4">
                    <label className="block text-sm text-slate-400 mb-1">
                      Zoom: {activeCard.imageTransform.scale.toFixed(2)}x
                    </label>
                    <input
                      type="range"
                      min="1"
                      max="5"
                      step="0.1"
                      value={activeCard.imageTransform.scale}
                      onChange={(e) =>
                        updateCard({
                          imageTransform: {
                            ...activeCard.imageTransform,
                            scale: parseFloat(e.target.value),
                          },
                        })
                      }
                    ...
                    <input
                      ...
                      max="100"
                      step="1"
                      value={activeCard.imageTransform.x}
                      onChange={(e) =>
                        updateCard({
                          imageTransform: {
                            ...activeCard.imageTransform,
                    ...
                    <input
                      ...
                      max="100"
                      step="1"
                      value={activeCard.imageTransform.y}
                      onChange={(e) =>
                        updateCard({
                          imageTransform: {
                            ...activeCard.imageTransform,
            ...
            <SidebarSection title="Export Settings">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  className="w-4 h-4 rounded text-blue-500 bg-slate-600 border-slate-500"
                />
      ...
      <footer className="bg-slate-800 border-t border-slate-700 p-4 flex-shrink-0">
        <div className="flex items-center gap-4 overflow-x-auto pb-2">
          <button
            ...
            className="flex-shrink-0 w-24 h-36 border-2 border-dashed border-slate-500 rounded-lg flex flex-col items-center justify-center text-slate-400 hover:border-blue-500 hover:text-blue-500 hover:bg-slate-700 disabled:opacity-50"
          >
            <Plus size={24} />
            ...
            <div
              ...
              className={
                ...
                (card.id === activeCardId
                  ? "ring-2 ring-offset-2 ring-offset-slate-800 ring-blue-500"
                  : "ring-1 ring-slate-600")
              }
              onClick={() => setActiveCardId(card.id)}
            >
              <div
                className="w-24 h-36 bg-cover bg-center rounded-lg overflow-hidden flex flex-col justify-between p-1 text-white text-shadow-sm"
                style={{ backgroundColor: card.colors.border }}
              >
                <div
                  className="p-1 text-xs text-center font-bold truncate"
                  style={{
                    color: card.title.color,
                    backgroundColor: card.colors.titleBar,
                    borderRadius: "4px",
                  }}
                >
                  {card.title.text}
              ...
              <div className="absolute top-1 right-1 flex flex-col gap-1">
                <button
                  ...
                  className="bg-slate-100 rounded-full p-1 shadow hover:bg-slate-200 disabled:opacity-50"
                  disabled={cards.length >= 8}
                >
                  <Copy size={12} />
                </button>
                {cards.length > 1 && (
                  <button
                    ...
                    className="bg-slate-100 rounded-full p-1 shadow hover:bg-red-200"
                  >
